...
public class ConfigItemModel {
    ...
    public void refreshConfigurationValues(){
        ...
        try {
            ...
            mNagAmount = Integer.parseInt(ConfigItemRepository.getConfigItem(NagAmount).getValue());
            mPayDateFromCourtDate = Integer.parseInt(ConfigItemRepository.getConfigItem(PayDateFromCourtDate).getValue());
            mCourtDateFromNow = Integer.parseInt(ConfigItemRepository.getConfigItem(CourtDateFromNow).getValue());
            mOffenceMinutesFromNow = Integer.parseInt(ConfigItemRepository.getConfigItem(OffenceMinutesFromNow).getValue());
            mTmtApiUser = ConfigItemRepository.getConfigItem(TmtApiUser).getValue();
            mMinTickets = Integer.parseInt(ConfigItemRepository.getConfigItem(MinTickets).getValue());
            mTicketBookSize = Integer.parseInt(ConfigItemRepository.getConfigItem(TicketBookSize).getValue());
            mGpsInterval = Integer.parseInt(ConfigItemRepository.getConfigItem(GpsInterval).getValue());
            mPaymentDetailHeading = ConfigItemRepository.getConfigItem(paymentDetailHeading).getValue();
            mPaymentDetailInfo = ConfigItemRepository.getConfigItem(paymentDetailInfo).getValue();
