...
namespace Kapsch.ITS.Repositories
{
    public class VerifyRepository
    {
        public struct sAddressInfo
        {
            ...
            public string mAddressDate;
            public string mUserDetails;
        ...
        public VerifyRepository(DataContext dbContext, long credentialID, string credentialUserName)
        {
        ...
        public List<sAddressInfo> GetAdditionalAddress(int personID)
        {
            List<sAddressInfo> addressInfos = new List<sAddressInfo>();

            try
            {
                OracleParameter par;
                var cmd = new OracleCommand("pkg_address_management.get_additional_address", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;

                // Inputs
                par = new OracleParameter("p_person_id", personID);
                par.Direction = ParameterDirection.Input;
                par.OracleDbType = OracleDbType.Int32;
                cmd.Parameters.Add(par);

                cmd.Parameters.Add(new OracleParameter("O_RESULT_CURSOR", OracleDbType.RefCursor, 0, ParameterDirection.Output, true, 0, 0, "", DataRowVersion.Current, "O_RESULT_CURSOR"));
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drPhysAddressInfo in ds.Tables[0].Rows)
                    {
                        sAddressInfo addressInfo = new sAddressInfo();
                        addressInfo.mCode = drPhysAddressInfo["CODE"].ToString();
                        addressInfo.mDate = Convert.ToDateTime(drPhysAddressInfo["ADDRESS_DATE"]);
                        addressInfo.mPOBox = drPhysAddressInfo["PO_BOX"].ToString();
                        addressInfo.mResidual = drPhysAddressInfo["RESIDUAL"].ToString();
                        addressInfo.mResidualScore = Convert.ToInt32(drPhysAddressInfo["SCORE"]);
                        addressInfo.mStreet = drPhysAddressInfo["STREET"].ToString();
                        addressInfo.mSuburb = drPhysAddressInfo["SUBURB"].ToString();
                        addressInfo.mTown = drPhysAddressInfo["TOWN"].ToString();
                        addressInfo.AddressTypeID = Convert.ToInt32(drPhysAddressInfo["ADDRESS_TYPE_ID"]);

                        addressInfo.mAddressDate = drPhysAddressInfo["ADDRESS_DATE"].ToString();
                        addressInfo.mCompanyName = drPhysAddressInfo["COMPANY_NAME"].ToString();
                        addressInfo.mIDNumber = drPhysAddressInfo["ID_NUMBER"].ToString();
                        addressInfo.mPersonID = Convert.ToInt32(drPhysAddressInfo["PERSON_ID"]);
                        addressInfo.mResidualScore = Convert.ToInt32(drPhysAddressInfo["SCORE"]);
                        addressInfo.mSource = drPhysAddressInfo["SOURCE"].ToString();
                        addressInfo.mUserDetails = drPhysAddressInfo["USER_DETAILS"].ToString();
        ...
        public void addressInitialise(out sAddressInfo addr)
        {
            ...
            addr.mUserDetails = string.Empty;
            addr.mSource = string.Empty;
            addr.mPersonID = -1;
            addr.mIDNumber = string.Empty;
            addr.mCompanyName = string.Empty;
            addr.mAddressDate = null;
            addr.mCompanyName = string.Empty;
            addr.mAddressDate = string.Empty;
            addr.AddressTypeID = 0;
        }
        ...
        public bool getPostalCodes(string city, string suburb, string code, bool isPhysical, List<cPostalCode> codes)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("LOOKUPS.get_postal_codes", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool caseFirst(ref List<sRejectReasons> reasons, ref List<sCaptureTypes> types, out int count, ref sCaseInfo caseInfo)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.get_lookup_data", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool caseAccept(ref sCaseInfo caseInfo, bool addrChanged, bool personChanged, int typeID, decimal typeAmount, ref List<sCaptureTypes> types, out int count, int printImageID)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.accept_veri_multi_image", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool caseReject(ref sCaseInfo caseInfo, int reasonID, bool regnoChanged, bool addrChanged, bool personChanged, int typeID, decimal typeAmount, ref List<sCaptureTypes> types, out int count)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.reject_veri_multi_image", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool checkAddress(ref sCaseInfo caseInfo, bool forSummons)
        {
            ...
            try
            {
                ...
                if (forSummons)
                    cmd = new OracleCommand("summons_verification.check_address", DbConnection);
                else
                    cmd = new OracleCommand("offence_verification.check_address", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool caseUnlock(string ticketNo)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.UNLOCK_VERIFICATION_USER", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool fishpondGetCases(ref List<sRejectReasons> reasons, ref List<cFishpondInfo> cases)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.get_fishpond_lookup_data", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool fishpondGetCase(string ticketNo, ref sCaseInfo caseInfo, ref List<sCaptureTypes> types)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.lock_fishpond_user", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool fishpondCaseReject(sCaseInfo caseInfo, int reasonID, bool regnoChanged, bool addrChanged, bool personChanged, int typeID, decimal typeAmount)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("OFFENCE_VERIFICATION_NATIS.reject_verification_fishpond", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool fishpondCaseAccept(sCaseInfo caseInfo, bool addrChanged, bool personChanged, int typeID, decimal typeAmount, int printImageID)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.accept_verification_fishpond", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool fishpondUnlock(string ticketNo)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("offence_verification.unlock_fishpond_user", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool summonsLock(ref sCaseInfo caseInfo)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("summons_verification.lock_summons_feeder", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool summonsAccept(ref sCaseInfo caseInfo, bool addrChanged, bool personChanged)
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("summons_verification.save_ticket_data", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool summonsUnlock()
        {
            ...
            {
                OracleParameter par;
                var cmd = new OracleCommand("summons_verification.unlock_summons_feeder", DbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        /// <summary>
        /// Check if the number-plate exists on central.
        /// </summary>
        public string pNPCapturePath
        {
            get { return mNPCapturePath; }
        }
    }
}
